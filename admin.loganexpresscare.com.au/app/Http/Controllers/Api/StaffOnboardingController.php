...
class StaffOnboardingController extends ApiBaseController
{
    ...
    public function store()
    {
        ...
        $validator = Validator::make($request->all(), [
            'email' => 'required|email|max:255', // Required to find existing user
        ...
        try {
            // Try to find the existing user by email in Users table first
            // $user = User::where('email', $request->email)->first();
            
            // If not found in Users table, search in staff_onboarding table
            if (true) {
                $staffOnboarding = StaffOnboarding::where('email', $request->email)->first();
                
                if (!$staffOnboarding) {
                    return response()->json([
                        'success' => false,
                        'message' => 'User not found. Please ensure you have completed the initial signup process.',
                        'error_code' => 'USER_NOT_FOUND'
                    ], 404);
                }
                
                // Check if this staff onboarding already has documents uploaded
                if ($staffOnboarding->ndis_screening || $staffOnboarding->children_check) {
                    return response()->json([
                        'success' => false,
                        'message' => 'User already has completed onboarding'
                    ], 409);
                }
                
                // Update existing staff onboarding record
                $uploadedFiles = $this->handleFileUploads($request);
                
                $staffOnboarding->update([
            ...
            // Create the staff onboarding record linked to existing user
